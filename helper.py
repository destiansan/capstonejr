...
def plot_automake(data):
    ...
    ax = pd.concat([auto_fraud,auto_nfraud],axis=1).plot.bar(color =['#c34454','#53a4b1'], 
        title = "Fraud Reported by Auto Make ", 
        figsize=(10, 10))
    # Plot Configuration
    plt.legend(['fraud','not fraud'], bbox_to_anchor=(1, 1))
...
def plot_witness(data):
    ...
    ax = pd.concat([witness_fraud,witness_nfraud],axis=1).plot.bar(stacked = 'witnesses',
        color =['#c34454','#53a4b1'], 
...
def plot_agesex(data):
    fraud_data = data[data['fraud_reported'] == 'Y']
    ax = pd.crosstab(index=fraud_data['age_group'],
...
def plot_citysev(data):
    fraud_data = data[data['fraud_reported'] == 'Y']
    ax = pd.crosstab(index=fraud_data['incident_city'],
           columns=fraud_data['incident_severity'],
           ).plot.barh(stacked = 'incident_city', 
           title = "Fraud Reported by Incident Severity and City ", 
           figsize=(8, 6))
